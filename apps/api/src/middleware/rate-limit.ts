...
const initializeRedisClient = () => {
  ...
  try {
    ...
    redisClient.on("error", (err: Error) => {
      logger.error("Rate limit Redis client error", "rate-limit", err);
    });

    redisClient.on("connect", () => {
      logger.info("Rate limit Redis client connected", "rate-limit");
    });

    return redisClient;
  } catch (error) {
    logger.error(
      "Failed to initialize rate limit Redis client",
      "rate-limit",
      error,
    );
    return null;
    ...
    if (useRedis && redisClient) {
      try {
        ...
        if (results && results.length === 2) {
          ...
          // If TTL is -1, key exists but has no expiration (first request or expired)
          if (ttl === -1 || count === 1) {
...
/**
 * Create rate limiting middleware from ConfigService values
 */
export const createConfiguredRateLimit = async (
  tier: "default" | "upload" | "auth" | "strict",
) => {
  let windowMs: number;
  ...
  switch (tier) {
    case "upload":
      windowMs = 60 * 60 * 1000; // 1 hour
      maxRequests = await configService.getRateLimitUpload();
      break;
    case "auth":
      windowMs = 15 * 60 * 1000; // 15 minutes
      maxRequests = await configService.getRateLimitAuth();
      break;
    case "strict":
      windowMs = 15 * 60 * 1000; // 15 minutes
    ...
    default:
      windowMs = 15 * 60 * 1000; // 15 minutes
      maxRequests = await configService.getRateLimitDefault();
  }
