...
export async function getCachedStats(dictionaryId: string): Promise<CacheEntry | null> {
...
export async function cachingMiddleware(c: Context, next: Function) {
  ...
  if (duration > 100) {
    ...
    logger.info(`Dictionary cache stats: ${stats.total} entries, ${stats.entries.filter(e => e.age < CACHE_TTL_SECONDS).length} active`);
  }
}
