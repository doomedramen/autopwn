...
export async function runHashcatAttack({
  ...
  try {
    ...
    const validatedAttackFile = await ensureAttackFileExists(handshakePath, attackMode, network.bssid!)
    ...
    const validatedHandshakePath = await validateFilePath(validatedAttackFile, {
      allowedBasePaths: ['temp/handshakes', 'data/handshakes', 'uploads/handshakes', 'temp/pmkids'],
    ...
    logger.info('Prepared hashcat command', 'hashcat', {
      jobId,
      attackMode,
      command: hashcatCommand.replace(/\b([a-fA-F0-9]{32,})\b/g, '[REDACTED_HASH]') // Redact potential hashes in logs
    })
...
async function executeHashcat(
  ...
  try {
    ...
    childProcess.stdout?.on('data', (chunk: Buffer) => {
      ...
      const now = Date.now()
      if (progressCallback && now - lastProgressUpdate > 5000) { // Update every 5 seconds max
        // Simple progress estimation based on elapsed time
        const estimatedProgress = Math.min(95, Math.floor((now - startTime) / 36000)) // Assume max 1 hour
...
export async function checkHashcatAvailability() {
  ...
  try {
    ...
    const { stdout: modesOutput } = await execAsync('hashcat --help | grep -E "(16800|22000)" || true')
    const supportedModes = modesOutput.includes('16800') && modesOutput.includes('22000')
    ...
    const errorMessage = error instanceof Error ? error.message : 'hashcat: command not found'
 ...
 */
async function ensureAttackFileExists(handshakePath: string, attackMode: 'pmkid' | 'handshake', bssid: string): Promise<string> {
  ...
  try {
    ...
    logger.info('Attack file does not exist, attempting extraction', 'hashcat', {
      handshakePath,
      attackMode,
      bssid
    })
    ...
    const pcapPath = handshakePath.replace(/\/(handshakes|pmkids)\/.*$/, '/original.pcap')
    ...
    try {
      ...
      throw new Error(`Attack file not found and extraction failed: ${extractError instanceof Error ? extractError.message : 'Unknown error'}`)
    }
  }
}
  ...
  try {
    ...
    try {
      ...
      if ((stats.mode & 0o002) !== 0) {
        ...
        recommendations.push('Remove world-write permissions from temp directory')
      }
    } catch (error) {
