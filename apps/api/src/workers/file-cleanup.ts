      ...
      const deletionPromises = batch.map(async (filePath) => {
        try {
          ...
          if (stats.isFile()) {
            ...
            logger.debug('Skipping non-file path', 'file-cleanup', { filePath })
...
async function findOldFiles(olderThan: Date, userId?: string): Promise<string[]> {
  ...
  const dictionaryFiles = oldDictionaries.map(d => d.filePath).filter(Boolean)
...
async function cleanEmptyDirectory(dir: string): Promise<boolean> {
  try {
    ...
    if (files.length === 0) {
      ...
      await fs.rmdir(dir)
      logger.debug('Removed empty directory', 'file-cleanup', { directory: dir })
      return true
    }
    ...
    if (remainingFiles.length === 0) {
      await fs.rmdir(dir)
      logger.debug('Removed directory after cleaning subdirectories', 'file-cleanup', {
        directory: dir,
        subdirsCleaned
      })
      return true
...
async function updateDatabaseForDeletedFiles(deletedFiles: string[]) {
  ...
  for (const filePath of deletedFiles) {
    try {
      ...
      const updateResult = await db.update(dictionaries)
        .set({
          filePath: null,
          status: 'failed', // File was deleted, so dictionary is now in failed state
      ...
      if (updateResult.length > 0) {
        ...
        logger.debug('Updated dictionary records for deleted file', 'file-cleanup', {
          filePath,
      ...
      // Note: networks table doesn't have filePath field, so we can't update it directly
      // In a real implementation, you might want to add a filePath field to networks
      // or use a different mechanism to track network file associations
...
export const cleanupStrategies = {
  ...
  async cleanupOldFiles(userId?: string) {
    try {
      logger.info('Starting old files cleanup strategy', 'file-cleanup', { userId })
  ...
  async cleanupFailedJobs(userId?: string) {
    try {
      logger.info('Starting failed jobs cleanup strategy', 'file-cleanup', { userId })
  ...
  async cleanupCompletedJobs(userId?: string) {
    try {
      logger.info('Starting completed jobs cleanup strategy', 'file-cleanup', { userId })
