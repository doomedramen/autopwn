 ...
 */
export const dictionaryProcessingConfigSchema = z.object({
  // Generation options
  baseWords: z.array(z.string()).optional().describe('Base words for dictionary generation'),
  ...
  transformations: z.array(z.enum([
    ...
    'prepend_numbers',
    'prepend_special'
  ])).optional().describe('Text transformations to apply'),

  // Upload processing options
  encoding: z.string().optional().default('utf-8').describe('File encoding'),
  deduplication: z.boolean().optional().default(true).describe('Remove duplicate entries'),
  minWordLength: z.number().int().min(1).optional().default(1).describe('Minimum word length'),
  maxWordLength: z.number().int().max(256).optional().default(256).describe('Maximum word length'),

  // Advanced options
  customRules: z.record(z.string()).optional().describe('Custom transformation rules'),
  metadata: z.record(z.unknown()).optional().describe('Additional metadata'),
}).strict().describe('Dictionary processing configuration')

export type DictionaryProcessingConfig = z.infer<typeof dictionaryProcessingConfigSchema>
...
export const jobConfigSchema = z.object({
  ...
  attackMode: z.enum(['pmkid', 'handshake', 'dictionary', 'mask', 'hybrid', 'combinator'])
    .describe('Hashcat attack mode'),
  hashcatMode: z.number().int().min(0).max(99999)
    .describe('Hashcat hash mode (e.g., 22000 for WPA)'),

  // Target information
  targetFile: z.string().optional().describe('Path to target file (handshake/hash)'),
  ...
  // Hashcat options
  workloadProfile: z.number().int().min(1).max(4).optional().default(3)
    .describe('Hashcat workload profile (1=low, 4=nightmare)'),
  optimizedKernel: z.boolean().optional().default(true).describe('Use optimized kernel (-O)'),
  rulesFile: z.string().optional().describe('Path to rules file'),
  mask: z.string().optional().describe('Mask for mask attack (e.g., ?a?a?a?a?a?a?a?a)'),

  // Resource limits
  runtime: z.number().int().min(0).optional().default(3600)
    .describe('Maximum runtime in seconds'),
  sessionTimeout: z.number().int().min(0).optional()
    .describe('Session timeout in seconds'),

  // Advanced options
  customArgs: z.array(z.string()).optional().describe('Additional hashcat arguments'),
  gpuDevices: z.array(z.number().int()).optional().describe('GPU device IDs to use'),
  cpuAffinity: z.array(z.number().int()).optional().describe('CPU cores to use'),

  // Metadata
  priority: z.enum(['low', 'normal', 'high', 'critical']).optional().default('normal')
    .describe('Job priority in queue'),
  tags: z.array(z.string()).optional().describe('Job tags for organization'),
  metadata: z.record(z.unknown()).optional().describe('Additional metadata'),
}).strict().describe('Job configuration')
...
export const jobResultSchema = z.object({
  ...
  crackedPassword: z.string().optional().describe('The cracked password (if found)'),
  passwordsFound: z.number().int().min(0).optional().default(0)
    .describe('Number of passwords found'),

  // Performance metrics
  processingTime: z.number().int().min(0).describe('Total processing time in milliseconds'),
  hashesTotal: z.number().int().min(0).optional().describe('Total number of hashes processed'),
  hashesCracked: z.number().int().min(0).optional().describe('Number of hashes cracked'),
  recoveredPercentage: z.number().min(0).max(100).optional()
  ...
  }).optional().describe('Cracking speed'),

  devices: z.array(z.object({
    id: z.number().int().describe('Device ID'),
    name: z.string().describe('Device name'),
    type: z.enum(['CPU', 'GPU']).describe('Device type'),
    temperature: z.number().int().optional().describe('Temperature in Celsius'),
    utilization: z.number().min(0).max(100).optional().describe('Utilization percentage'),
  ...
  errorDetails: z.record(z.unknown()).optional().describe('Additional error details'),

  // Output files
  outputFiles: z.array(z.object({
    type: z.enum(['potfile', 'output', 'log', 'restore']).describe('File type'),
    path: z.string().describe('File path'),
    size: z.number().int().min(0).optional().describe('File size in bytes'),
  })).optional().describe('Generated output files'),

  // Additional metadata
  exitCode: z.number().int().optional().describe('Hashcat exit code'),
  metadata: z.record(z.unknown()).optional().describe('Additional result metadata'),
}).strict().describe('Job result data')
...
export const jobResultDataSchema = z.object({
  ...
  type: z.enum(['password', 'hash', 'handshake', 'error', 'progress'])
  ...
  timestamp: z.string().datetime().optional().describe('When this result was generated'),
  processingTime: z.number().int().min(0).optional().describe('Time to crack in milliseconds'),
  keyspace: z.object({
    position: z.number().int().min(0).describe('Current keyspace position'),
    total: z.number().int().min(0).optional().describe('Total keyspace size'),
  }).optional().describe('Keyspace progress information'),

  // Progress updates (for type='progress')
  progress: z.object({
    percentage: z.number().min(0).max(100).describe('Progress percentage'),
    currentSpeed: z.number().min(0).optional().describe('Current cracking speed'),
    timeRemaining: z.number().int().min(0).optional().describe('Estimated time remaining in seconds'),
    status: z.string().optional().describe('Current status message'),
  }).optional().describe('Progress information'),
  ...
  // Additional metadata
  metadata: z.record(z.unknown()).optional().describe('Additional result metadata'),
}).strict().describe('Job result entry data')
 ...
 */
export function validateDictionaryProcessingConfig(data: unknown): DictionaryProcessingConfig | null {
  if (!data) return null

  const result = dictionaryProcessingConfigSchema.safeParse(data)
  if (!result.success) {
    console.error('Dictionary processing config validation failed:', result.error.format())
    return null
  }
...
export function validateJobConfig(data: unknown): JobConfig {
  ...
  if (!result.success) {
    throw new Error(`Job config validation failed: ${JSON.stringify(result.error.format())}`)
  }
...
export function validateJobResultData(data: unknown): JobResultData {
  ...
  if (!result.success) {
    throw new Error(`Job result data validation failed: ${JSON.stringify(result.error.format())}`)
  }
