...
import { Job, JobItem, Result, Dictionary, PcapEssidMapping } from '@autopwn/shared';
...
export function addPcapEssidMapping(pcapFilename: string, essid: string, bssid?: string): PcapEssidMapping {
  console.log('[DEBUG] Adding PCAP ESSID mapping:', { pcapFilename, essid, bssid });
  const database = getWriteDb();
  try {
    const stmt = database.prepare(
      'INSERT OR REPLACE INTO pcap_essid_mapping (pcap_filename, essid, bssid) VALUES (?, ?, ?)'
    );
    const result = stmt.run(pcapFilename, essid, bssid);
    console.log('[DEBUG] PCAP ESSID mapping inserted with ID:', result.lastInsertRowid);
    const getStmt = database.prepare('SELECT * FROM pcap_essid_mapping WHERE id = ?');
    const mapping = getStmt.get(result.lastInsertRowid) as PcapEssidMapping;
    console.log('[DEBUG] PCAP ESSID mapping retrieved from database:', mapping);
    return mapping;
  } catch (error: any) {
    console.error('[DEBUG] Failed to add PCAP ESSID mapping:', error?.message || error);
    throw error;
  }
}

export function getPcapEssidMappings(pcapFilename?: string): PcapEssidMapping[] {
  const database = getDb();
  let stmt;
  if (pcapFilename) {
    stmt = database.prepare('SELECT * FROM pcap_essid_mapping WHERE pcap_filename = ? ORDER BY essid ASC');
    return stmt.all(pcapFilename) as PcapEssidMapping[];
  } else {
    stmt = database.prepare('SELECT * FROM pcap_essid_mapping ORDER BY pcap_filename, essid ASC');
    return stmt.all() as PcapEssidMapping[];
  }
}

export function deletePcapEssidMappings(pcapFilename: string): void {
  console.log('[DEBUG] Deleting PCAP ESSID mappings for:', pcapFilename);
  const database = getWriteDb();
  try {
    const stmt = database.prepare('DELETE FROM pcap_essid_mapping WHERE pcap_filename = ?');
    const result = stmt.run(pcapFilename);
    console.log('[DEBUG] Deleted PCAP ESSID mappings:', result.changes);
  } catch (error: any) {
    console.error('[DEBUG] Failed to delete PCAP ESSID mappings:', error?.message || error);
    throw error;
  }
}
...
export function addJob(jobData: {
  ...
  console.log('[DEBUG] Adding job to database:', jobData);
  const database = getWriteDb();
  try {
    const stmt = database.prepare(`
      INSERT INTO jobs (
        job_id, filename, status, priority, items_total, captures, total_hashes, created_at
    ...
    const result = stmt.run(
      ...
      jobData.status,
      jobData.priority || 0,
      jobData.items_total || null,
      jobData.captures || null,
      jobData.total_hashes || null
    );
    console.log('[DEBUG] Job inserted with ID:', result.lastInsertRowid);
    const getStmt = database.prepare('SELECT * FROM jobs WHERE id = ?');
    const job = getStmt.get(result.lastInsertRowid) as Job;
    console.log('[DEBUG] Job retrieved from database:', job);
    return job;
  } catch (error: any) {
    console.error('[DEBUG] Failed to add job to database:', error?.message || error);
    throw error;
  }
}

export function addJobDictionaries(jobId: number, dictionaryIds: number[]): void {
  console.log('[DEBUG] Adding job dictionaries:', { jobId, dictionaryIds });
  const database = getWriteDb();
  try {
    const stmt = database.prepare(
    ...
    for (const dictId of dictionaryIds) {
      stmt.run(jobId, dictId, 'pending');
    }
    console.log('[DEBUG] Job dictionaries added successfully');
  } catch (error: any) {
    console.error('[DEBUG] Failed to add job dictionaries:', error?.message || error);
    throw error;
  }
}
