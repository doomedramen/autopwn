...
export default function JobQueue() {
  ...
  const retryJob = async (job: Job) => {
    // Using a custom confirmation dialog would be better, but for now just proceed
    try {
      ...
      if (res.ok) {
        ...
        toast.success("Job has been reset to pending status");
      } else {
  ...
  const togglePause = async (job: Job) => {
    try {
      ...
      if (!res.ok) {
        const error = await res.json();
        toast.error(`Failed to toggle pause: ${error.error}`);
      }
    } catch (error) {
  ...
  const setPriority = async (job: Job, priority: number) => {
    try {
      ...
      if (!res.ok) {
        const error = await res.json();
        toast.error(`Failed to set priority: ${error.error}`);
      }
    } catch (error) {
  ...
  const openRetryModal = () => {
    if (selectedJobs.size === 0) {
      toast.error("Please select at least one failed job to retry");
      return;
    }
  ...
  const retrySelectedJobs = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(`Retry batch job created successfully! ${result.message}`);
        setShowRetryModal(false);
        setSelectedJobs(new Set());
        setSelectedDictionaries(new Set());
      } else {
        const error = await res.json();
        toast.error(`Failed to create retry batch: ${error.error}`);
      }
    } catch (error) {
      toast.error("Failed to create retry batch");
    }
  };
