...
export function useFileUpload(options: UseFileUploadOptions) {
  ...
  const calculateOverallProgress = useCallback((uploads: FileUploadState['uploads']) => {
  ...
  const pollProgress = useCallback(async (fileId: string) => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          setState(prev => ({
            ...
            overallProgress: calculateOverallProgress(prev.uploads.map(u =>
          ...
          if (progress.stage === 'completed' || progress.stage === 'error') {
            ...
            if (progress.stage === 'completed') {
              try {
                const uploadResponse = await fetch(`${getUploadEndpoint(options.type)}?fileId=${fileId}`, {
                  method: 'GET',
                  headers: {
                ...
                setState(prev => ({
                  ...
                  uploads: prev.uploads.map(upload =>
                    upload.fileId === fileId
                      ? { ...upload, result, status: result.success ? 'completed' : 'error' }
                      : upload
                  ),
                  isUploading: prev.uploads.some(u =>
                    u.fileId !== fileId && (u.status === 'uploading' || u.status === 'processing')
                  )
                }));
  ...
  const selectFiles = useCallback((files: FileList | File[]) => {
    ...
    if (options.maxFiles && (state.files.length + validFiles.length) > options.maxFiles) {
  ...
  const uploadFiles = useCallback(async (files?: File[]) => {
    const filesToUpload = files || state.files.filter(f =>
      !state.uploads.some(u => u.file === f && u.status !== 'pending')
    );
      ...
      try {
        ...
        if (result.success && result.data?.fileId) {
          ...
          setState(prev => ({
            ...
            uploads: prev.uploads.map(upload =>
              upload.file === file
                ? { ...upload, status: 'error' as const, error: result.error || result.message }
                : upload
            ),
            isUploading: prev.uploads.some(u => u.status === 'uploading' || u.status === 'processing')
          }));

          options.onError?.(file.name, result.error || result.message || 'Upload failed');
    ...
    const allComplete = state.uploads.every(u =>
    ...
    try {
      ...
      if (response.ok) {
        setState(prev => ({
          ...
          uploads: prev.uploads.map(upload =>
            upload.fileId === fileId
              ? { ...upload, status: 'error' as const, error: 'Upload cancelled' }
              : upload
          ),
          isUploading: prev.uploads.some(u => u.status === 'uploading' || u.status === 'processing')
        }));
  ...
  const clearCompleted = useCallback(() => {
    setState(prev => ({
      ...
      files: prev.files.filter(file =>
        !prev.uploads.some(upload =>
          upload.file === file && (upload.status === 'completed' || upload.status === 'error')
        )
      ),
      uploads: prev.uploads.filter(upload =>
  ...
  const clearAll = useCallback(() => {
    ...
    state.uploads.forEach(upload => {
      if (upload.fileId && (upload.status === 'uploading' || upload.status === 'processing')) {
...
function validateFileSize(file: File, config: {
  maxSize: number;
