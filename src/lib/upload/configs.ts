    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        return {
          ...
          summary: {
            ...
            message: networksWithHandshakes > 0
      ...
      return {
        ...
        analysis: {
          ...
          networks: [],
          isValid: false,
          errorMessage: error instanceof Error ? error.message : 'Failed to process PCAP file'
        },
        summary: {
          totalNetworks: 0,
          networksWithHandshakes: 0,
...
export const DICTIONARY_UPLOAD_CONFIG: UploadConfig = {
  ...
  customProcessing: async (filePath: string, _config: UploadConfig) => {
    ...
    try {
      ...
      if (!isCompressed && stats.size < 500 * 1024 * 1024) { // < 500MB
        try {
          ...
          if (lineCount > 0) {
            ...
            const sampleInterval = Math.max(1, Math.floor(lineCount / maxSamples));

            for await (const line of rl) {
              const trimmedLine = line.trim();
              if (trimmedLine && trimmedLine.length > 0) {
                if (sampleCount % sampleInterval === 0 && sampleWords.length < maxSamples) {
 ...
 */
export function validateUploadConfig(config: UploadConfig): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
